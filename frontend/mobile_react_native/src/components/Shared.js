  ...
  return (
    ...
    >
      <View style={s.heroRow}>
        {leading}
        <View style={{ flex: 1 }}>
          <Text style={s.heroTitle}>{titre}</Text>
          {sousTitre ? <Text style={s.heroSub}>{sousTitre}</Text> : null}
        </View>
        {trailing}
      </View>
    </LinearGradient>
...
export function NavTile({ icon, label, sub, accent, trailing, onPress }) {
  return (
    <TouchableOpacity style={[s.navTile, { borderLeftColor: accent }]} onPress={onPress}>
      ...
      <View style={{ flex: 1 }}>
        <Text style={s.navTileLabel}>{label}</Text>
        {sub ? <Text style={s.navTileSub}>{sub}</Text> : null}
      </View>
      {trailing}
    </TouchableOpacity>
  );
  ...
  return (
    <TouchableOpacity onPress={onPress} disabled={loading}>
      <LinearGradient colors={heroGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.primaryBtn}>
        <Text style={s.primaryBtnText}>{loading ? '...' : label}</Text>
      </LinearGradient>
    </TouchableOpacity>
  );
}
  ...
  return (
    <TouchableOpacity style={s.listItem} onPress={onPress}>
      <View style={s.listItemTop}>
        <Text style={s.listItemTitre}>{titre}</Text>
        {tag}
      </View>
      <Text style={s.listItemSub}>{sousTitre}</Text>
    </TouchableOpacity>
  );
}
